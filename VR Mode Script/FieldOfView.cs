...
public class FieldOfView : MonoBehaviour
{
	...
	{Debug.Log (collider.transform.name);
		...
		if (collider.transform.name == "SoldierRed") {
			c = Color.red; 
			meshRenderer.material.color = new Color (c.r, c.g, c.b, .2f);
			
			StartCoroutine( setGreenBack());
			
			hhmm = Instantiate( Hhmm, transform.position+Vector3.up*.9f, Quaternion.identity) as Transform;
