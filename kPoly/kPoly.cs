...
*/
		...
		public static Mesh Create_Cube (string name, int xseg, int yseg, int zseg, float width, float height, float depth,int _windingIndex=0,int _pivotIndex=0, int _colliderIndex=1)
		{	
			Mesh m = new Mesh ();
			...
			for (int fi=0; fi<6; fi ++) {
				m = Create_Mesh (name, xseg, yseg, width, height, fi, _windingIndex, _pivotIndex, _colliderIndex);
				verts.AddRange (m.vertices);
				Vector3 fv  = Facing(fi);
				for( int i=0; i<verts.Count; i++ ){
					verts[i] = verts[i] + fv; 
				}
				trias.AddRange (m.triangles);
				uvs.AddRange (m.uv);
			}
...
*/
		...
		{
			GameObject quad = new GameObject ((name != "") ? name : "kPolyCube");
		...
		public static Mesh Create_Plane (string name, int xseg, int yseg,
                                        ...
                                        int facingIndex = 0, int windingIndex = 0, int pivotIndex = 0, int colliderIndex = 1)
		{
			return Create_Mesh (name, xseg, yseg, width, height, facingIndex, windingIndex, pivotIndex, colliderIndex);
		...
		public static GameObject Create_Plane_Object (string name, int xseg, int yseg,
                                        ...
                                        int facingIndex = 0, int windingIndex = 0, int pivotIndex = 0, int colliderIndex = 1)
		{
			GameObject quad = new GameObject ((name != "") ? name : "kPolyPlane");
			quad.AddComponent<MeshRenderer> ();
			Mesh m = Create_Mesh (name, xseg, yseg, width, height, facingIndex, windingIndex, pivotIndex, colliderIndex);
			((MeshFilter)quad.AddComponent<MeshFilter> ()).sharedMesh = m;
			return quad;
		}
        #endregion
